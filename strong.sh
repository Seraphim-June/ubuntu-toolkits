#!/bin/bash
chmod 777 ./*
for d in /usr/bin /usr/sbin /usr/local/bin /usr/local/sbin; do ln -s -f $(pwd)/gu "$d/"; done
for d in /usr/bin /usr/sbin /usr/local/bin /usr/local/sbin; do ln -s -f $(pwd)/rrmm "$d/"; done
